...
class Trigger(BaseModel):
    ...
    sta_sec: float = Field(default=0.5, gt=0, le=10.0, description="Short-Term Average window")
    lta_sec: float = Field(default=10.0, gt=0, le=300.0, description="Long-Term Average window")

    # Trigger thresholds
    thr_on: float = Field(default=3.5, gt=1.0, description="Ratio required to start a trigger")
    thr_off: float = Field(default=1.5, gt=0, description="Ratio required to end a trigger")
